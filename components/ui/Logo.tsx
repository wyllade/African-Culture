...
export function Logo({ className = 'h-7 w-7', variant = 'default' }: LogoProps) {
  const stroke = variant === 'white' ? '#fff' : '#b94a32'
  return (
    <svg viewBox="0 0 32 32" fill="none" className={className} aria-hidden="true">
      <circle cx="16" cy="16" r="14" stroke={stroke} strokeWidth="2" opacity="0.3" />
      <path
      ...
      <path
        ...
        fillOpacity="0.15"
      />
      <path d="M11 16h10M16 11v10" stroke={stroke} strokeWidth="1.5" strokeLinecap="round" opacity="0.5" />
    </svg>
  )
}
